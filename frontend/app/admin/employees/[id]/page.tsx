...
export default function EmployeeDetailPage() {
  ...
  const handleClearDebt = async () => {
    ...
    try {
      ...
      if (res.ok) {
        fetchEmployee() // Recarga para que la deuda pase a 0
      } else {
  ...
  if (loading) return <div className="p-10 text-center font-bold text-blue-600 animate-pulse">Lade Mitarbeiterdaten...</div>
  ...
  return (
    <div className="p-10 space-y-8 bg-slate-50/50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-3xl border shadow-sm">
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              <button onClick={() => setIsEditOpen(true)} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-[10px] font-black uppercase rounded-xl transition-all">
                Vertrag bearbeiten
              </button>
              <button onClick={() => setIsTimeBlockOpen(true)} className="px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 text-[10px] font-black uppercase rounded-xl transition-all flex items-center gap-1">
                <span className="material-symbols-outlined text-[14px]">more_time</span> Zeit buchen
              </button>
              <button onClick={handleExportPDF} className="px-4 py-2 bg-slate-900 text-white text-[10px] font-black uppercase rounded-xl hover:bg-black transition-all flex items-center gap-1 shadow-md">
                <span className="material-symbols-outlined text-[14px]">picture_as_pdf</span> PDF Export
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6 border-l border-slate-100 pl-6 w-full md:w-auto">
          <div className="text-right">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Vertrag</p>
            <p className="text-xl font-black text-slate-800">{contractedHours}h</p>
          </div>

          {debtHours > 0 && (
            <div className="text-right relative">
              <div className="flex items-center justify-end gap-1 mb-1">
                <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest">Schulden (Deuda)</p>
                <button 
              ...
              <p className="text-xl font-black text-rose-600">+{debtHours}h</p>
            </div>
          )}

          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 text-right min-w-[120px]">
            <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest leading-tight">Soll diese Woche<br/>(Total a asignar)</p>
            <p className="text-3xl font-black text-slate-900 mt-1">{targetThisWeek}h</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className={`p-6 rounded-2xl border shadow-sm ${calculatedStats && calculatedStats.balance < 0 ? 'bg-orange-50 border-orange-100' : 'bg-emerald-50 border-emerald-100'}`}>
          <p className="text-[10px] font-black uppercase text-slate-500 mb-2">Zeitkonto (Balance)</p>
          <p className={`text-3xl font-black ${calculatedStats && calculatedStats.balance < 0 ? 'text-orange-600' : 'text-emerald-600'}`}>
            {calculatedStats?.balance.toFixed(2)}h
          </p>
          <p className="text-[10px] opacity-70 mt-1 text-slate-600">Differenz zum Wochen-Soll</p>
        </div>
        ...
        <div className="bg-blue-600 p-6 rounded-2xl border border-blue-700 shadow-lg shadow-blue-200 relative overflow-hidden">
          ...
          <span className="material-symbols-outlined absolute -right-4 -bottom-4 text-8xl text-blue-500 opacity-30">payments</span>
        </div>
      </div>
      ...
      {isEditOpen && <EditEmployeeModal employee={employee} onClose={() => setIsEditOpen(false)} onUpdated={fetchEmployee} />}
      {isTimeBlockOpen && <AddTimeBlockModal employeeId={employee.id} onClose={() => setIsTimeBlockOpen(false)} onAdded={fetchEmployee} />}
