...
function canRequestCancellation(status: string) {
  ...
  return !["COMPLETED", "DONE", "FINISHED", "CANCELLED", "FAILED"].includes(
    normalized
  )
}
...
function toModalService(item: ServiceHistoryItem) {
  return {
    id: Number(item.id),
    date: item.dateIso,
    startTime: item.dateIso,
    serviceCode: {
      code: item.code,
...
export default function ClientHistoryPage() {
    ...
    async function loadHistory() {
      try {
        ...
        if (!res.ok) {
          ...
          setError(message || "Verlauf konnte nicht geladen werden.")
          return
        }
  ...
  function openRequestModal(type: RequestType, service: ServiceHistoryItem) {
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      const matchesSearch =
        ...
        item.primaryProfessional.toLowerCase().includes(searchLower) ||
        item.professionals.join(" ").toLowerCase().includes(searchLower)
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6 lg:px-8">
      <section className="mx-auto max-w-7xl">
          ...
          <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm font-medium text-slate-500">
                Eine Übersicht Ihrer vergangenen und geplanten Services.
              </p>
            </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[1.3fr_220px]">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Nach Service oder Mitarbeiter suchen..."
        ...
        {!error && filteredItems.length === 0 && (
          <div className="mt-6 rounded-[2rem] border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm">
            ...
            <p className="mt-2 text-sm font-medium text-slate-500">
              Für die aktuelle Suche oder Kategorie gibt es keine Services.
            </p>
          </div>
        )}
                ...
                <tbody>
                  {paginatedItems.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-5 align-top">
                        <span className="inline-flex rounded-xl bg-slate-100 px-3 py-2 text-xs font-black text-slate-700">
                          {item.code}
                        </span>
                      </td>
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          <div className="flex flex-wrap justify-end gap-2">
                            <button
                              type="button"
                              onClick={() =>
                                openRequestModal("SERVICE_ISSUE", item)
                              }
                              className="rounded-xl bg-rose-50 px-3 py-2 text-xs font-black text-rose-600 transition-all hover:bg-rose-100"
                            >
                              Problem
                            </button>
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                openRequestModal("CHANGE_REQUEST", item)
                              }
                              className="rounded-xl bg-amber-50 px-3 py-2 text-xs font-black text-amber-700 transition-all hover:bg-amber-100"
                            >
                              Änderung
                            </button>

                            {canRequestCancellation(item.status) && (
                              <button
                                type="button"
                                onClick={() =>
                                  openRequestModal(
                                ...
                                className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black text-slate-700 transition-all hover:bg-slate-200"
                              >
                                Stornierung
                              </button>
                            )}
              ...
              >
                ...
                <div className="mt-5 flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={() => openRequestModal("SERVICE_ISSUE", item)}
                      className="rounded-2xl bg-rose-50 px-4 py-3 text-xs font-black text-rose-600"
                    >
                      Problem melden
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => openRequestModal("CHANGE_REQUEST", item)}
                      className="rounded-2xl bg-amber-50 px-4 py-3 text-xs font-black text-amber-700"
                    >
                      Änderung
                    </button>
                    ...
                    {canRequestCancellation(item.status) && (
                      <button
                        type="button"
                        onClick={() =>
                          openRequestModal("CANCELLATION_REQUEST", item)
                        }
                        className="col-span-2 rounded-2xl bg-slate-100 px-4 py-3 text-xs font-black text-slate-700"
                      >
          ...
          <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] bg-white shadow-2xl">
            ...
            <div className="flex flex-col gap-3 border-t border-slate-100 px-6 py-5">
              <div className="grid gap-2 sm:grid-cols-3">
                <button
                  type="button"
                  onClick={() => openRequestModal("SERVICE_ISSUE", selectedItem)}
                  className="inline-flex items-center justify-center gap-2 rounded-2xl border border-rose-100 bg-rose-50 px-4 py-3 text-sm font-black text-rose-600 transition-all hover:bg-rose-100"
                >
                  <span className="material-symbols-outlined text-[20px]">
                    report
                  </span>
                ...
                <button
                  type="button"
                  onClick={() =>
                    openRequestModal("CHANGE_REQUEST", selectedItem)
                  }
                  className="inline-flex items-center justify-center gap-2 rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3 text-sm font-black text-amber-700 transition-all hover:bg-amber-100"
                >
                  <span className="material-symbols-outlined text-[20px]">
                    edit_calendar
                  </span>
                ...
                {canRequestCancellation(selectedItem.status) && (
                  <button
                    type="button"
                    onClick={() =>
                      openRequestModal("CANCELLATION_REQUEST", selectedItem)
                    }
      ...
      {requestModal && (
        <ClientRequestModal
          ...
          service={toModalService(requestModal.service)}
